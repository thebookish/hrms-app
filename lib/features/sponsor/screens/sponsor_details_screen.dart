...
class _EmployeeSponsorViewScreenState
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading
        ...
        child: Column(
          children: [
            _buildHeader(sponsor!),
            const SizedBox(height: 20),
            _buildDetailTile('Sponsor Name', sponsor!.name),
            _buildDetailTile('Industry', sponsor!.industry),
            _buildDetailTile('Contact Person', sponsor!.contactPerson),
            // _buildDetailTile('Email', sponsor!.email),
            _buildDetailTile('Phone', sponsor!.phone),
            _buildDetailTile('Address', sponsor!.address),
          ],
        ),
      ),
    );
  ...
  Widget _buildHeader(SponsorModel sponsor) {
    return Column(
      children: [
        ...
        Text(
          ...
          style: const TextStyle(
              fontSize: 22, fontWeight: FontWeight.bold, color: AppColors.brandColor),
        ),
      ],
    );
  }
  ...
  Widget _buildDetailTile(String label, String? value) {
    return Card(
      ...
      child: ListTile(
        leading: const Icon(Icons.info_outline, color: AppColors.brandColor),
        title: Text(label),
